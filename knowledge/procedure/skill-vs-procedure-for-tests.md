...
- **Procedure** (`procedures/test-unit/implement.md`): a full agent workflow that uses the MCP tool `ts_test_strategist`, runs `ts_checker`, and loops until tests pass. Invoked via `perclst start --procedure test-unit/implement`.
...
- Use the `test-unit/implement` procedure when delegating test creation to a sub-agent that should iterate until passing.
