...
- **Procedure** (`procedures/test-unit/implement-unit-test.md`): a full agent workflow that uses the MCP tool `ts_test_strategist`, runs `ts_checker`, and loops until tests pass. Invoked via `perclst start --procedure test-unit/implement-unit-test`.
...
- Use the `test-unit/implement-unit-test` procedure when delegating test creation to a sub-agent that should iterate until passing.
