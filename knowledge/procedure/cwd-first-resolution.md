...
`ProcedureRepository` resolves procedure files by name (e.g. `meta-curate-knowledge` →
`procedures/meta-curate-knowledge.md`). Projects that use perclst outside its own repo need
...
1. `<workingDir>/procedures/<name>.md` — project-local, checked first
2. `<package>/procedures/<name>.md` — built-in fallback
...
- Place custom procedures in `<project-root>/procedures/<name>.md` to shadow or extend
  built-in ones
